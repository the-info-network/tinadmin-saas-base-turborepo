...
/**
 * Get tenant ID for support operations
 ...
 */
export async function getTenantForSupport(): Promise<string> {
  ...
  if (authError || !user) {
    console.error("[getTenantForSupport] Auth error:", authError);
  ...
  if (userError || !userData) {
    console.error("[getTenantForSupport] Error fetching user data:", userError);
  ...
  if (isPlatformAdmin) {
    ...
    if (error) {
      console.error("[getTenantForSupport] Error fetching tenants:", {
        code: error.code,
    ...
    if (!tenants || tenants.length === 0) {
      ...
      throw new Error(
        "No tenants found. As a Platform Admin, you need to create a tenant first before creating support tickets. " +
